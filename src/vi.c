...
static void search_again(int reverse);
...
#define NAVIGATE_MODE                                                                                                  \
    4 /* Navigate the spreadsheet while editing                                                                        \
a line */
...
void write_line(int c)
{
    ...
    if (mode == EDIT_MODE)
    {
        ...
        case 'D':
            ...
            break;
        case 'E':
            linelim = for_word(arg, 1, 1, 0);
            break;
        case 'F':
            linelim = find_char(arg, -1);
            break;
        case 'G':
            if (histp > 0)
                histp = lasthist;
        ...
        case 'R':
            ...
            break;
        case 'T':
            linelim = to_char(arg, -1);
            break;
        case 'W':
            linelim = for_word(arg, 0, 1, 0);
            break;
        case 'X':
            u_save(c);
            back_space();
            break;
        case 'Y':
            yank_chars(linelim, (int)strlen(line), 0);
            break;
        case 'a':
            u_save(c);
            append_line();
    ...
    else if (mode == INSERT_MODE)
    {
        ...
        case (ctl('b')):
            if (numeric_field)
            {
                if (linelim == (int)strlen(line) && (line[linelim - 1] == '+' || line[linelim - 1] == '-'))
                {
                    toggle_navigate_mode();
                    backcol(1);
                }
    ...
    else if (mode == REP_MODE)
    {
        ...
        case KEY_BACKSPACE:
        case (ctl('h')):
            if (linelim > (int)strlen(undo_line))
                back_space();
            else
            {
                linelim = back_line(1);
    ...
    else if (mode == NAVIGATE_MODE)
    {
        ...
        case (ctl('i')):
            ...
            else if (linelim == (int)strlen(line) && (line[linelim - 1] == '+' || line[linelim - 1] == '-' ||
                                                      (line[linelim - 1] == ' ' && line[linelim - 2] == '=')))
            {
                ins_string("@sum(");
                showdr();
                ins_in_line(')');
        ...
        case 'v': { /* insert variable value */
            ...
            {
                (void)sprintf(temp, "%.*f", precision[curcol], p->v);
                ins_string(temp);
                toggle_navigate_mode();
            }
        }
        ...
        case 'c':
            ...
            {
                ins_string(r_name(cr->r_left->row, cr->r_left->col, cr->r_right->row, cr->r_right->col));
                toggle_navigate_mode();
                ins_in_line(' ');
                showrange = 0;
            }
        ...
        case 'f':
            ...
            {
                ins_string(r_name(fr->or_left->row, fr->or_left->col, fr->or_right->row, fr->or_right->col));
                toggle_navigate_mode();
                ins_in_line(' ');
                showrange = 0;
            }
        ...
        case 'r':
            if ((fr = find_frange(currow, curcol)))
            {
                ins_string(r_name(fr->ir_left->row, fr->ir_left->col, fr->ir_right->row, fr->ir_right->col));
                toggle_navigate_mode();
                ins_in_line(' ');
                showrange = 0;
            }
...
void dotab(void)
{
        ...
        (completethis && line[linelim - 1] == ' '))
    {
        if (!completethis)
        {
            for (completethis = line + linelim - 1; isalnum(*completethis) || *completethis == '_';
                 completethis--) /* */
                ;
            completethis++;
            len = (int)(line + linelim - completethis);
            if (!find_range(completethis, -len, NULL, NULL, &lastmatch))
            {
                firstmatch = lastmatch;
                while (firstmatch->r_next && !strncmp(completethis, firstmatch->r_next->r_name, (size_t)len))
        ...
        {
            len = (int)(line + linelim - completethis);
            memmove(completethis, line + linelim, (size_t)((int)strlen(line) - linelim + 1));
            linelim -= len;
...
static void u_save(int c)
{
    ...
    {
        undolen = (unsigned)(strlen(line) + 40);
...
static void restore_it(void)
{
    ...
    {
        templen = (unsigned)(strlen(line) + 40);
 ...
 */
static int for_line(int narg, int stop_null)
{
    int cpos = linelim;

    if (linelim < 0)
        return (linelim);
    else if (linelim + narg <= (int)strlen(line))
        cpos += narg;
    else
        cpos = (int)strlen(line);
...
static int for_word(int narg, int end_word, int big_word, int stop_null)
{
    register int c;
    register int cpos;
    ...
    while (cpos < (int)strlen(line) && narg--)
    {
        if (end_word)
            cpos++;
...
static int back_word(int narg, int big_word)
{
    register int c;
    register int cpos;
    ...
    while (cpos > 0 && narg--)
    {
        ...
        else if (cpos > 0 && (line[cpos - 1] == ' ' || (istext(line[cpos]) && !istext(line[cpos - 1])) ||
 ...
 */
static void del_in_line(int narg, int back_null)
{
    register int len, i;
    ...
    if (linelim >= 0)
    {
        ...
        if (linelim == len && linelim > 0)
            linelim--;
        strncpy(putbuf, line + linelim, narg);
        putbuf[narg] = '\0';
...
void doabbrev(void)
{
    ...
    if (!(isalpha(line[linelim - 1]) || isdigit(line[linelim - 1]) || line[linelim - 1] == '_') ||
    ...
    if (isalpha(line[pos]) || isdigit(line[pos]) || line[pos] == '_')
    {
        ...
        for (; pos >= istart; pos--)
            if (isalpha(line[pos]) || isdigit(line[pos]) || line[pos] == '_' || line[pos] == ' ')
                break;
    pos++;
...
static void change_case(int narg)
{
    if (linelim < 0)
    {
        linelim = 0;
    ...
    while (narg--)
    {
        if (islower(line[linelim]))
            line[linelim] = (char)toupper(line[linelim]);
        else if (isupper(line[linelim]))
            line[linelim] = (char)tolower(line[linelim]);
...
static int get_motion(int change)
{
    ...
    case 'b':
        return (back_word(arg, 0));
    case 'B':
        return (back_word(arg, 1));
    case 'c':
        return (change ? -1 : linelim);
    case 'd':
        return (!change ? -1 : linelim);
    case 'e':
        return (for_word(arg, 1, 0, 1) + 1);
    case 'E':
        return (for_word(arg, 1, 1, 1) + 1);
    case 'f':
        return ((c = find_char(arg, 1)) == linelim ? c : c + 1);
    case 'F':
        return (find_char(arg, -1));
    case 'h':
        return (back_line(arg));
    case 'l':
        return (for_line(arg, 1));
    case 't':
        return ((c = to_char(arg, 1)) == linelim ? c : c + 1);
    case 'T':
        return (to_char(arg, -1));
    case 'w':
        return (for_word(arg, change, 0, 1) + change);
    case 'W':
        return (for_word(arg, change, 1, 1) + change);
    default:
        return (linelim);
    }
}
...
static void save_hist(void)
{
    if (!lasthist || strcmp(history[lasthist].histline, line))
    {
        ...
        {
            history[lasthist].len = (unsigned)(strlen(line) + 40);
            history[lasthist].histline = scxrealloc(history[lasthist].histline, history[lasthist].len);
        }
...
static void back_hist(void)
{
    if (histp == 0)
    {
        ...
        {
            history[0].len = (unsigned)(strlen(line) + 40);
            history[0].histline = scxrealloc(history[0].histline, history[0].len);
        }
...
static void search_again(int reverse)
{
    ...
    do
    {
        ...
        look_here = line;
        do_next = 0;
        while ((look_here = (char *)strchr(look_here, *last_search)) != NULL && !found_it && !do_next)
        {
...
static int find_char(int narg, int n)
{
    register int i;
...
static int to_char(int narg, int n)
{
    register int i;
    int tmp = linelim;
...
void remember(int save)
{
    ...
    if (save && (currow != remrow || curcol != remcol || strow != remstrow || stcol != remstcol))
    {
        savedrow[0] = remrow;
        savedcol[0] = remcol;
...
void gohome(void)
{
    ...
    if ((fr = find_frange(currow, curcol)))
    {
        if (currow >= fr->ir_left->row && currow <= fr->ir_right->row && curcol >= fr->ir_left->col &&
            curcol <= fr->ir_right->col && (currow > fr->ir_left->row || curcol > fr->ir_left->col))
        {
            currow = fr->ir_left->row;
            curcol = fr->ir_left->col;
        }
        else if (currow > fr->or_left->row || curcol > fr->or_left->col)
        {
...
void leftlimit(void)
{
    ...
    if ((fr = find_frange(currow, curcol)))
    {
        if (currow >= fr->ir_left->row && currow <= fr->ir_right->row && curcol > fr->ir_left->col &&
            curcol <= fr->ir_right->col)
            curcol = fr->ir_left->col;
        else if (curcol > fr->or_left->col && curcol <= fr->or_right->col)
            curcol = fr->or_left->col;
        else
            curcol = 0;
    }
...
void rightlimit(void)
{
    ...
    if ((fr = find_frange(currow, curcol)))
    {
        ...
        {
            curcol = maxcols - 1;
            while (!VALID_CELL(p, currow, curcol) && curcol > fr->or_right->col)
                curcol--;
            if ((fr = find_frange(currow, curcol)))
                curcol = fr->or_right->col;
        }
...
void gototop(void)
{
    ...
    if ((fr = find_frange(currow, curcol)))
    {
        if (curcol >= fr->ir_left->col && curcol <= fr->ir_right->col && currow > fr->ir_left->row &&
            currow <= fr->ir_right->row)
            currow = fr->ir_left->row;
        else if (currow > fr->or_left->row && currow <= fr->or_right->row)
            currow = fr->or_left->row;
        else
            currow = 0;
    }
...
void gotobottom(void)
{
    ...
    if ((fr = find_frange(currow, curcol)))
    {
        ...
        {
            currow = maxrows - 1;
            while (!VALID_CELL(p, currow, curcol) && currow > fr->or_right->row)
                currow--;
            if ((fr = find_frange(currow, curcol)))
                currow = fr->or_right->row;
        }
