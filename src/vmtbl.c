...
#define GROWALLOC(newptr, oldptr, nelem, type, msg)                                                                    \
    newptr = (type *)(void *)scxrealloc((char *)oldptr, (unsigned)((size_t)(nelem) * sizeof(type)));                   \
    if (newptr == (type *)NULL)                                                                                        \
    {                                                                                                                  \
        error(msg);                                                                                                    \
        return (FALSE);                                                                                                \
...
int growtbl(int rowcol, int toprow, int topcol)
{
    ...
    if ((rowcol == GROWCOL) || (rowcol == GROWBOTH) || (rowcol == GROWNEW))
    {
        ...
        for (i = 0; i < maxrows; i++)
        {
            if ((tbl[i] = (struct ent **)(void *)scxrealloc(
                     (char *)tbl[i], (unsigned)((size_t)newcols * sizeof(struct ent **)))) == (struct ent **)0)
            {
                error(nowider);
                return (FALSE);
            }
    ...
    for (; i < newrows; i++)
    {
        if ((tbl[i] = (struct ent **)(void *)scxmalloc((unsigned)((size_t)newcols * sizeof(struct ent **)))) ==
            (struct ent **)0)
        {
            error(nowider);
            return (FALSE);
