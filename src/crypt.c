...
void creadfile(char *save, int eraseflg)
{
    ...
    {                           /* if child		 */
        ...
        (void)fprintf(stderr, "execl(%s, \"crypt\", %s, 0) in creadfile() failed", CRYPT_PATH, KeyWord);
        exit(-127);
    }
    else
    { /* else parent */
...
int cwritefile(char *fname, int r0, int c0, int rn, int cn)
{
...
#ifdef DOBACKUPS
    if (!backup_file(busave) && (yn_ask("Could not create backup copy, Save anyway?: (y,n)") != 1))
        return (0);
#endif
    ...
    {                           /* if child		 */
        ...
        (void)fprintf(stderr, " ");
        (void)execl(CRYPT_PATH, "crypt", KeyWord, 0);
        (void)fprintf(stderr, "execl(%s, \"crypt\", %s, 0) in cwritefile() failed", CRYPT_PATH, KeyWord);
        exit(-127);
    }
    else
    { /* else parent */
