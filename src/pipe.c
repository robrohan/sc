    ...
    *line = '\0';
    if ((fr = find_frange(currow, curcol)))
    {
        ...
        sprintf(line + strlen(line), "%s",
                r_name(fr->ir_left->row, fr->ir_left->col, fr->ir_right->row, fr->ir_right->col));
    }
    strcat(line, "\n");
    write(fd, line, strlen(line));
    linelim = -1;
...
void getrange(char *name, int fd)
{
    ...
    *line = '\0';
    ...
    {
        sprintf(line, "%s%s%s%d", r->r_left.vf & FIX_COL ? "$" : "", coltoa(r->r_left.vp->col),
        ...
        if (r->r_is_range)
        {
            ...
            while (*p)
                p++;
            sprintf(p, ":%s%s%s%d", r->r_right.vf & FIX_COL ? "$" : "", coltoa(r->r_right.vp->col),
