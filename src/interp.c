...
extern int decimal; /* Set if there was a decimal point in the number */
...
static double fn1_eval(double (*fn)(double), double farg);
...
static double doindex(int minr, int minc, int maxr, int maxc, struct enode *val)
{
    ...
    if (c <= maxc && c >= minc && r <= maxr && r >= minr && (p = *ATBL(tbl, r, c)) && p->flags & is_valid)
    {
        if (p->cellerror)
            cellerror = CELLINVALID;
        return p->v;
...
static double dotime(int which, double when)
{
    ...
    {
    case HOUR:
        return ((double)(tm_cache.tm_hour));
    case MINUTE:
        return ((double)(tm_cache.tm_min));
    case SECOND:
        return ((double)(tm_cache.tm_sec));
    case MONTH:
        return ((double)(tm_cache.tm_mon));
    case DAY:
        return ((double)(tm_cache.tm_mday));
    case YEAR:
        return ((double)(tm_cache.tm_year));
    }
    /* Safety net */
    cellerror = CELLERROR;
    return ((double)0);
...
static struct ent *getent(char *colstr, double rowdoub)
{
    ...
    if (((row = (int)floor(rowdoub)) >= 0) && (row < maxrows) /* in range */
        && ((collen = (int)strlen(colstr)) <= 2)              /* not too long */
        && ((col = atocol(colstr, collen)) >= 0) && (col < maxcols))
    { /* in range */
        p = *ATBL(tbl, row, col);
...
double eval(register struct enode *e)
{
    ...
    {
    case '+':
        return (eval(e->e.o.left) + eval(e->e.o.right));
    case '-': {
        double l, r;
        l = eval(e->e.o.left);
        r = eval(e->e.o.right);
    ...
    case '^':
        return (fn2_eval(pow, eval(e->e.o.left), eval(e->e.o.right)));
    case '<':
        return (eval(e->e.o.left) < eval(e->e.o.right));
    case '=': {
        double l, r;
    ...
    case '>':
        return (eval(e->e.o.left) > eval(e->e.o.right));
    case '&':
        return (eval(e->e.o.left) != 0.0 && eval(e->e.o.right) != 0.0);
    case '|':
        return (eval(e->e.o.left) != 0.0 || eval(e->e.o.right) != 0.0);
    case IF:
    case '?':
        return eval(e->e.o.left) != 0.0 ? eval(e->e.o.right->e.o.left) : eval(e->e.o.right->e.o.right);
    case 'm':
        return (-eval(e->e.o.left));
    case 'f': {
        int rtmp = rowoffset;
        int ctmp = coloffset;
        double ret;
    ...
    case 'F':
        return (eval(e->e.o.left));
    case '!':
        return (eval(e->e.o.left) == 0.0);
    case ';':
        return (((int)eval(e->e.o.left) & 7) + (((int)eval(e->e.o.right) & 7) << 3));
    case O_CONST:
        if (!isfinite(e->e.k))
        {
            e->op = ERR_;
    ...
    case VLOOKUP: {
        ...
        if (minr > maxr)
            r = maxr, maxr = minr, minr = r;
        if (minc > maxc)
            c = maxc, maxc = minc, minc = c;
        switch (e->op)
        {
        case LOOKUP:
            return dolookup(e->e.o.right, minr, minc, maxr, maxc, 1, minc == maxc);
        case HLOOKUP:
            return dolookup(e->e.o.right->e.o.left, minr, minc, maxr, maxc, (int)eval(e->e.o.right->e.o.right), 0);
        case VLOOKUP:
            return dolookup(e->e.o.right->e.o.left, minr, minc, maxr, maxc, (int)eval(e->e.o.right->e.o.right), 1);
        case INDEX:
    ...
    case ABS:
        return (fn1_eval(fabs, eval(e->e.o.left)));
    case ACOS:
        return (fn1_eval(acos, eval(e->e.o.left)));
    case ASIN:
        return (fn1_eval(asin, eval(e->e.o.left)));
    case ATAN:
        return (fn1_eval(atan, eval(e->e.o.left)));
    case ATAN2:
        return (fn2_eval(atan2, eval(e->e.o.left), eval(e->e.o.right)));
    case CEIL:
        return (fn1_eval(ceil, eval(e->e.o.left)));
    case COS:
        return (fn1_eval(cos, eval(e->e.o.left)));
    case EXP:
        return (fn1_eval(exp, eval(e->e.o.left)));
    case FABS:
        return (fn1_eval(fabs, eval(e->e.o.left)));
    case FLOOR:
        return (fn1_eval(floor, eval(e->e.o.left)));
    case HYPOT:
        return (fn2_eval(hypot, eval(e->e.o.left), eval(e->e.o.right)));
    case LOG:
        return (fn1_eval(log, eval(e->e.o.left)));
    case LOG10:
        return (fn1_eval(log10, eval(e->e.o.left)));
    case POW:
        return (fn2_eval(pow, eval(e->e.o.left), eval(e->e.o.right)));
    case SIN:
        return (fn1_eval(sin, eval(e->e.o.left)));
    case SQRT:
        return (fn1_eval(sqrt, eval(e->e.o.left)));
    case TAN:
        return (fn1_eval(tan, eval(e->e.o.left)));
    case DTR:
        return (dtr(eval(e->e.o.left)));
    case RTD:
        return (rtd(eval(e->e.o.left)));
    case RND:
        if (rndtoeven)
    ...
    case PV:
    case PMT:
        return (finfunc(e->op, eval(e->e.o.left), eval(e->e.o.right->e.o.left), eval(e->e.o.right->e.o.right)));
    case HOUR:
        return (dotime(HOUR, eval(e->e.o.left)));
    case MINUTE:
        return (dotime(MINUTE, eval(e->e.o.left)));
    case SECOND:
        return (dotime(SECOND, eval(e->e.o.left)));
    case MONTH:
        return (dotime(MONTH, eval(e->e.o.left)));
    case DAY:
        return (dotime(DAY, eval(e->e.o.left)));
    case YEAR:
        return (dotime(YEAR, eval(e->e.o.left)));
    case NOW:
        return (dotime(NOW, (double)0.0));
    case DTS:
        return (dodts((int)eval(e->e.o.left), (int)eval(e->e.o.right->e.o.left), (int)eval(e->e.o.right->e.o.right)));
    case TTS:
        return (dotts((int)eval(e->e.o.left), (int)eval(e->e.o.right->e.o.left), (int)eval(e->e.o.right->e.o.right)));
    case STON:
        return (doston(seval(e->e.o.left)));
    case EQS:
        return (doeqs(seval(e->e.o.right), seval(e->e.o.left)));
    case LMAX:
        return dolmax(e);
    case LMIN:
        return dolmin(e);
    case NVAL:
        return (donval(seval(e->e.o.left), eval(e->e.o.right)));
    case MYROW:
    ...
    case PI_:
        return ((double)M_PI);
    case BLACK:
        return ((double)COLOR_BLACK);
    case RED:
        return ((double)COLOR_RED);
    case GREEN:
        return ((double)COLOR_GREEN);
    case YELLOW:
        return ((double)COLOR_YELLOW);
    case BLUE:
        return ((double)COLOR_BLUE);
    case MAGENTA:
        return ((double)COLOR_MAGENTA);
    case CYAN:
        return ((double)COLOR_CYAN);
    case WHITE:
        return ((double)COLOR_WHITE);
    default:
        error("Illegal numeric expression");
        exprerr = 1;
    }
    cellerror = CELLERROR;
    return ((double)0.0);
...
static char *docat(register char *s1, register char *s2)
{
    register char *p;
    char *arg1, *arg2;
...
static char *dodate(time_t tloc, char *fmtstr)
{
    char buff[FBUFLEN];
    char *p;
...
static char *doext(struct enode *se)
{
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (!se->e.o.s || strlen(buff) != strlen(se->e.o.s))
                        se->e.o.s = scxrealloc(se->e.o.s, (unsigned)strlen(buff));
    ...
    if (se->e.o.s)
        return (strcpy(scxmalloc((unsigned)(strlen(se->e.o.s) + 1)), se->e.o.s));
    else
...
static char *dosubstr(char *s, register int v1, register int v2)
{
    register char *s1, *s2;
    char *p;
...
char *seval(register struct enode *se)
{
    ...
    if (se == (struct enode *)0)
        return (char *)0;
    switch (se->op)
    {
    case O_SCONST:
        p = scxmalloc((unsigned)(strlen(se->e.s) + 1));
    ...
    case O_VAR: {
        ...
        if (!vp || !vp->label)
            return (NULL);
        p = scxmalloc((unsigned)(strlen(vp->label) + 1));
        (void)strcpy(p, vp->label);
        return (p);
    }
    case '#':
        return (docat(seval(se->e.o.left), seval(se->e.o.right)));
    case 'f': {
        int rtmp = rowoffset;
        int ctmp = coloffset;
        char *ret;
    ...
    case 'F':
        return (seval(se->e.o.left));
    case IF:
    case '?':
        return (eval(se->e.o.left) != 0.0 ? seval(se->e.o.right->e.o.left) : seval(se->e.o.right->e.o.right));
    case DATE:
        return (dodate((time_t)(eval(se->e.o.left)), seval(se->e.o.right)));
    case FMT:
        return (dofmt(seval(se->e.o.left), eval(se->e.o.right)));
    case UPPER:
        return (docase(UPPER, seval(se->e.o.left)));
    case LOWER:
        return (docase(LOWER, seval(se->e.o.left)));
    case CAPITAL:
        return (docapital(seval(se->e.o.left)));
    case STINDEX: {
        int r, c;
        int maxr, maxc;
        int minr, minc;
    ...
    case EXT:
        return (doext(se));
    case SVAL:
        return (dosval(seval(se->e.o.left), eval(se->e.o.right)));
    case SUBSTR:
        return (dosubstr(seval(se->e.o.left), (int)eval(se->e.o.right->e.o.left) - 1,
                         (int)eval(se->e.o.right->e.o.right) - 1));
    case COLTOA:
        return (strcpy(scxmalloc((unsigned)10), coltoa((int)eval(se->e.o.left))));
    case FILENAME: {
        ...
        if (n || s++ == NULL)
            s = curfile;
        p = scxmalloc((unsigned)(strlen(s) + 1));
        (void)strcpy(p, s);
        return (p);
    }
    default:
...
struct enode *new(int op, struct enode *a1, struct enode *a2)
{
    ...
    if (freeenodes)
    {
        ...
        p = (struct enode *)(void *)scxmalloc((unsigned)sizeof(struct enode));
    p->op = op;
    p->e.o.left = a1;
    p->e.o.right = a2;
    p->e.o.s = NULL;
...
void copy(struct ent *dv1, struct ent *dv2, struct ent *v1, struct ent *v2)
{
    ...
    if (v1)
    {
        ...
        maxsc = v2->col;
        if (minsr > maxsr)
            r = maxsr, maxsr = minsr, minsr = r;
        if (minsc > maxsc)
            c = maxsc, maxsc = minsc, minsc = c;
    }
    ...
    else if (showrange && !(showsr == currow && showsc == curcol && mindr == currow && mindc == curcol &&
...
void go_last(void)
{
    ...
    case G_NONE:
        ...
        break;
    case G_NUM:
        num_search(gs.g_n, gs.g_row, gs.g_col, gs.g_lastrow, gs.g_lastcol, gs.errsearch);
        break;
    case G_CELL:
...
void num_search(double n, int firstrow, int firstcol, int lastrow, int lastcol, int errsearch)
{
    ...
    while (1)
    {
        ...
        p = *ATBL(tbl, r, c);
        if (!col_hidden[c] && p && (p->flags & is_valid) && (errsearch || (p->v == n)) &&
...
void hide_row(int narg)
{
    ...
    FullUpdate++;
    row_hidden[narg] = TRUE;
}

void hide_col(int narg)
{
    if (narg < 0)
    {
        error("Invalid Range");
...
int constant(register struct enode *e)
{
    return (e == NULL || e->op == O_CONST || e->op == O_SCONST || (e->op == 'm' && constant(e->e.o.left)) ||
            (e->op != O_VAR && !(e->op & REDUCE) && constant(e->e.o.left) && constant(e->e.o.right) &&
             e->op != EXT /* functions look like constants but aren't */
             && e->op != NVAL && e->op != SVAL && e->op != NOW && e->op != MYROW && e->op != MYCOL &&
             e->op != LASTROW && e->op != LASTCOL && e->op != NUMITER && e->op != FILENAME && optimize));
}
...
void efree(struct enode *e)
{
    if (e)
    {
        if (e->op != O_VAR && e->op != O_CONST && e->op != O_SCONST && !(e->op & REDUCE))
        {
            efree(e->e.o.left);
            efree(e->e.o.right);
        }
...
static void decodev(struct ent_ptr v)
{
    ...
    {
        (void)sprintf(line + linelim, "%s%s%s%d", v.vf & FIX_COL ? "$" : "", coltoa(v.vp->col),
