...
struct ent *lookat(int row, int col)
{
    ...
    if (*pp == NULL)
    {
        if (freeents != NULL)
        {
            ...
            *pp = (struct ent *)(void *)scxmalloc((unsigned)sizeof(struct ent));
        if (row > maxrow)
            maxrow = row;
        if (col > maxcol)
            maxcol = col;
...
int main(int argc, char **argv)
{
    ...
    while ((c = getopt(argc, argv, "axmoncrCReP:W:vq")) != EOF)
    {
        ...
        case 'q':
            ...
            break;
        default:
            (void)fprintf(stderr, "Usage: %s [-acemnoqrxCR] [file...]\n", progname);
            exit(1);
        }
    }
        ...
        while (running)
        {
            ...
            if ((isascii(c) && (iscntrl(c) || (c == 020))) || /* iscntrl broken in OSx4.1 */
                ...
                case ctl('t'):
                    ...
                    case 'M':
                        autocalc ^= 1;
                        error("Automatic recalculation %sabled.", autocalc ? "en" : "dis");
                        break;
                    case 'o':
                    case 'O':
                        optimize ^= 1;
                        error("%sptimize expressions upon entry.", optimize ? "O" : "Do not o");
                        break;
                    case 'n':
                        numeric = (!numeric);
                    ...
                    case 'c':
                        ...
                        error("Cell highlighting %sabled.", showcell ? "en" : "dis");
                        --modflg; /* negate the modflg++ */
                        break;
                    case 'b':
                        braille ^= 1;
                        error("Braille enhancement %sabled.", braille ? "en" : "dis");
                        --modflg; /* negate the modflg++ */
                        break;
                    case 's':
                        cslop ^= 1;
                        error("Color slop %sabled.", cslop ? "en" : "dis");
                        break;
                    case 'C':
                        color = !color;
                        if (has_colors())
                    ...
                    case 'N':
                        colorneg = !colorneg;
                        error("Color changing of negative numbers %sabled.", colorneg ? "en" : "dis");
                        break;
                    case 'E':
                        colorerr = !colorerr;
                        error("Color changing of cells with errors %sabled.", colorerr ? "en" : "dis");
                        break;
                    case 'x':
                    case 'X':
#if defined(VMS) || defined(MSDOS) || !defined(CRYPT_PATH)
                    ...
                    case 'L':
                        autolabel = (!autolabel);
                        error("Autolabel %sabled.", autolabel ? "en" : "dis");
                        break;
                    case '$':
                        if (prescale == 1.0)
                        {
                    ...
                    case 'e':
                        extfunc = (!extfunc);
                        error("External functions %sabled.", extfunc ? "en" : "dis");
                        break;
                    case ESC:
                    case ctl('g'):
                        error("");
                    ...
                    case 'I':
                        ...
                        error("Autoinsert %sabled.", autoinsert ? "en" : "dis");
                        break;
                    case 'w':
                    case 'W':
                        autowrap = (!autowrap);
                        error("Autowrap %sabled.", autowrap ? "en" : "dis");
                        break;
                    case 'z':
                    case 'Z':
                        rowlimit = currow;
                ...
                case '\035': /* ^] */
                    ...
                    break;
                     ...
                     ((!numeric && linelim < 0) || (linelim >= 0 && (mode_ind == 'e' || mode_ind == 'v')) ||
                      edistate >= 0))
            {
            ...
            else if (c > KEY_F0 && c <= KEY_F(FKEYS))
            {
                ...
                case '.':
                    ...
                    numeric_field = 1;
                    (void)sprintf(line, "let %s = %c", v_name(currow, curcol), c);
                    linelim = (int)strlen(line);
                    insert_mode();
                    break;
                ...
                case 'r':
                    ...
                    case 'c':
                        ...
                        break;
                    case 'm':
                        (void)sprintf(line, "move [destination src_range] %s ", v_name(currow, curcol));
                        linelim = (int)strlen(line);
                        insert_mode();
                        write_line(ctl('v'));
                        break;
                    ...
                    case 'v':
                        if (rcqual == 'r')
                        {
                            ...
                            if ((fr = find_frange(currow, curcol)))
                                valueize_area(currow, fr->or_left->col, currow + arg - 1, fr->or_right->col);
                            else
                ...
                case '\\':
                    if (!locked_cell(currow, curcol))
                    {
                        ...
                        (void)sprintf(line, "label %s = \"", v_name(currow, curcol));
                        linelim = (int)strlen(line);
                        insert_mode();
                    }
                    break;
                ...
                case '<':
                    if (!locked_cell(currow, curcol))
                    {
                        ...
                        (void)sprintf(line, "leftstring %s = \"", v_name(currow, curcol));
                        linelim = (int)strlen(line);
                        insert_mode();
                    }
                    break;
                ...
                case '>':
                    if (!locked_cell(currow, curcol))
                    {
                        ...
                        (void)sprintf(line, "rightstring %s = \"", v_name(currow, curcol));
                        linelim = (int)strlen(line);
                        insert_mode();
                    }
                    break;
                ...
                case 'F': {
                    ...
                    if (p && p->format)
                    {
                        (void)sprintf(line, "fmt [format] %s \"%s", v_name(currow, curcol), p->format);
                        edit_mode();
                        linelim = (int)strlen(line) - 1;
                    }
                    else
                    {
                        (void)sprintf(line, "fmt [format] %s \"", v_name(currow, curcol));
                        insert_mode();
                        linelim = (int)strlen(line);
                    }
                    break;
                ...
                case 'P':
                    ...
                    if (strrchr(curfile, '.') != NULL)
                    {
                        ...
                        else if (scext != NULL && !strcmp((strrchr(curfile, '.') + 1), scext))
                        {
                            *strrchr(curfile, '.') = '\0';
                            strcpy(curfile + strlen(curfile) + strlen(scext) + 1, ".\0");
                        }
                    }
                    if (*curfile)
                        error("Default path is \"%s.%s\"", curfile, scext == NULL ? "sc" : scext);
                    c = *(curfile + strlen(curfile) + strlen(curfile + strlen(curfile) + 1));
                    *(curfile + strlen(curfile) + strlen(curfile + strlen(curfile) + 1)) = '\0';
                    curfile[strlen(curfile)] = (char)c;
                    linelim = (int)strlen(line);
                    insert_mode();
                    break;
                case 'M':
                ...
                case 'W':
                     ...
                     */
                    if (*curfile)
                        error("Default file is \"%s.%s\"", curfile, ascext == NULL ? "asc" : ascext);
                ...
                case 'T': /* tbl output */
                    ...
                    if (*curfile && tbl_style == 0)
                        error("Default file is \"%s.%s\"", curfile, tbl0ext == NULL ? "cln" : tbl0ext);
                    else if (*curfile && tbl_style == TBL)
                        error("Default file is \"%s.%s\"", curfile, tblext == NULL ? "tbl" : tblext);
                    else if (*curfile && tbl_style == LATEX)
                        error("Default file is \"%s.%s\"", curfile, latexext == NULL ? "lat" : latexext);
                    else if (*curfile && tbl_style == SLATEX)
                        error("Default file is \"%s.%s\"", curfile, slatexext == NULL ? "stx" : slatexext);
                    else if (*curfile && tbl_style == TEX)
                        error("Default file is \"%s.%s\"", curfile, texext == NULL ? "tex" : texext);
                    c = *(curfile + strlen(curfile) + strlen(curfile + strlen(curfile) + 1));
                    *(curfile + strlen(curfile) + strlen(curfile + strlen(curfile) + 1)) = '\0';
                    curfile[strlen(curfile)] = (char)c;
                    linelim = (int)strlen(line);
                ...
                case 'x':
                    if (calc_order == BYROWS)
                        eraser(lookat(currow, curcol), lookat(currow, curcol + arg - 1));
                    else
                        eraser(lookat(currow, curcol), lookat(currow + arg - 1, curcol));
                    break;
                case 'Q':
                case 'q':
                    running = 0;
                ...
                case 'c':
                    ...
                    error("");
                    {
                        ...
                        for (c1 = curcol; arg-- && c1 < maxcols; c1++)
                        {
                            ...
                            copyent(n, p, currow - savedrow[c], c1 - savedcol[c], 0, 0, maxrow, maxcol, 0);
                            n->flags |= is_changed;
                        }
                ...
                case '*': {
                    ...
                    if (c == 'a' || c == 'A')
                    {
                        sprintf(line, "addnote [target range] %s ", v_name(currow, curcol));
                        linelim = (int)strlen(line);
                        insert_mode();
                        write_line(ctl('v'));
                        error("");
