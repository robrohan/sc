...
void initcolor(int colornum)
{
    if (!colornum)
    {
        ...
        for (i = 0; i < 8; i++)
            cpairs[i] = (struct colorpair *)(void *)scxmalloc((unsigned)sizeof(struct colorpair));
    }
...
void change_color(int pair, struct enode *e)
{
    ...
    if (!cpairs[pair])
        cpairs[pair] = (struct colorpair *)(void *)scxmalloc((unsigned)sizeof(struct colorpair));
    cpairs[pair]->fg = v & 7;
    ...
    if (!pair)
    {
        if (color_base)
            for (r = color_base; r; r = r->r_next)
                if ((r->r_left->row == r_left->row) && (r->r_left->col == r_left->col) &&
                    (r->r_right->row == r_right->row) && (r->r_right->col == r_right->col))
                {
                    if (r->r_next)
...
struct crange *find_crange(int row, int col)
{
    ...
    if (color_base)
        for (r = color_base; r; r = r->r_next)
        {
            if ((r->r_left->row <= row) && (r->r_left->col <= col) && (r->r_right->row >= row) &&
                (r->r_right->col >= col))
                return r;
        }
    return 0;
...
void list_colors(FILE *f)
{
    ...
    while (r)
    {
        fprintf(f, "  %-32s %d\n", r_name(r->r_left->row, r->r_left->col, r->r_right->row, r->r_right->col),
                r->r_color);
        if (brokenpipe)
            return;
        r = r->r_prev;
...
void fix_colors(int row1, int col1, int row2, int col2, int delta1, int delta2)
{
    ...
    if (color_base)
        for (cr = color_base; cr; cr = cr->r_next)
        {
            ...
            {
                if (r1 != r2 && r1 >= row1 && r1 <= row2)
                    r1 = row2 - delta1;
                if (c1 != c2 && c1 >= col1 && c1 <= col2)
                    c1 = col2 - delta1;
            }

            if (!(fr && (c2 < fr->or_left->col || c2 > fr->or_right->col)))
            {
                if (r1 != r2 && r2 >= row1 && r2 <= row2)
                    r2 = row1 + delta2;
                if (c1 != c2 && c2 >= col1 && c2 <= col2)
                    c2 = col1 + delta2;
            }
