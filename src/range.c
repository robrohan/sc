...
void add_range(char *name, struct ent_ptr left, struct ent_ptr right, int is_range)
{
    ...
    if (!find_range(name, (int)strlen(name), (struct ent *)0, (struct ent *)0, &prev))
    {
...
void write_ranges(FILE *f)
{
    ...
    while (r)
    {
        (void)fprintf(f, "define \"%s\" %s%s%s%d", r->r_name, r->r_left.vf & FIX_COL ? "$" : "",
...
void list_ranges(FILE *f)
{
    ...
    while (r)
    {
        (void)fprintf(f, "  %-30s %s%s%s%d", r->r_name, r->r_left.vf & FIX_COL ? "$" : "", coltoa(r->r_left.vp->col),
                      r->r_left.vf & FIX_ROW ? "$" : "", r->r_left.vp->row);
        if (brokenpipe)
            return;
        if (r->r_is_range)
            (void)fprintf(f, ":%s%s%s%d\n", r->r_right.vf & FIX_COL ? "$" : "", coltoa(r->r_right.vp->col),
