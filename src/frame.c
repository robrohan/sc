...
void add_frange(struct ent *or_left, struct ent *or_right, struct ent *ir_left, struct ent *ir_right, int toprows,
                int bottomrows, int leftcols, int rightcols)
{
    struct frange *r;
    ...
    if (ir_left)
    {
        ...
        if (ir_left->row < or_left->row || ir_left->col < or_left->col || ir_right->row > or_right->row ||
        ...
        for (r = frame_base; r; r = r->r_next)
        {
            if ((r->or_left == or_left) && (r->or_right == or_right))
            {
                ...
                else
                {
                    ...
                    r->ir_left = lookat(r->or_left->row + toprows, r->or_left->col + leftcols);
                    r->ir_right = lookat(r->or_right->row - bottomrows, r->or_right->col - rightcols);
                }
...
struct frange *find_frange(int row, int col)
{
    ...
    if (frame_base)
        for (r = frame_base; r; r = r->r_next)
        {
            if ((r->or_left->row <= row) && (r->or_left->col <= col) && (r->or_right->row >= row) &&
                (r->or_right->col >= col))
                return r;
        }
    return 0;
