...
void update(int anychanged) /* did any cell really change in value? */
{
    ...
    if (!(fr && fr->or_left->row <= currow && /* If we've left the	    */
          ...
          fr->or_right->row >= currow && fr->or_right->col >= curcol))
    {
    ...
    if ((curcol != lastcol) || FullUpdate)
    {
        ...
        if (fwidth[curcol] > cols - rescol - 2)
        {
            ...
            doformat(curcol, curcol, cols - rescol - 2, precision[curcol], realfmt[curcol]);
        }
        ...
        if (stcol >= 0 && stcol <= curcol)
        {
            ...
            if (!frTooLarge && fr && curcol <= stcol + lcols && fr->ir_left->col >= stcol + lcols)
            {
        ...
        while (stcol < 0 || curcol < stcol || stcol + lcols - 1 < curcol ||
               (colsinrange != fwidth[curcol] && stcol != curcol && stcol + lcols - 1 < gs.g_lastcol))
        {
            ...
            else if (stcol >= 0 && fr && curcol >= fr->or_left->col && curcol <= fr->ir_left->col && stcol < curcol &&
                     curcol <= stcol + lcols + fr->ir_left->col - fr->or_left->col)
            {
            ...
            else
            {
                 ...
                 */
                colsinrange = (colsinrange > cols - rescol - flcols - frcols - 2 ? cols - rescol - flcols - frcols - 2
                                                                                 : colsinrange);
                col = (cols - rescol - flcols - frcols - colsinrange) / 2;
                stcol = curcol;
                for (i = curcol - 1;
                     i >= (fr ? fr->or_left->col + fleftcols : 0) && (col - fwidth[i] > 0 || col_hidden[i]); i--)
                {
                    stcol--;
                    if (col_hidden[i])
                        continue;
    ...
    if ((currow != lastrow) || FullUpdate)
    {
        ...
        if (strow >= 0 && strow <= currow)
        {
            ...
            if (!frTooLarge && fr && currow <= strow + rows && fr->ir_left->row >= strow + rows)
            {
        ...
        while (strow < 0 || currow < strow || strow + rows - 1 < currow || strow + rows < currow + rowsinrange)
        {
            ...
            else if (strow >= 0 && fr && currow >= fr->or_left->row && currow <= fr->ir_left->row && strow < currow &&
                     currow <= strow + rows + fr->ir_left->row - fr->or_left->row)
            {
                while ((strow + rows < fr->ir_left->row && !frTooLarge) ||
                       (rowsinrange > 1 && strow != currow && strow + rows - 1 < gs.g_lastrow))
                {
                    if (row_hidden[++strow])
                        rows--;
                }
            ...
            else
            {
                 ...
                 */
                rowsinrange =
                    (rowsinrange > lines - RESROW - ftrows - fbrows ? lines - RESROW - ftrows - fbrows : rowsinrange);
                row = (lines - RESROW - ftrows - fbrows - rowsinrange) / 2;
                strow = currow;
                for (i = currow - 1; i >= (fr ? fr->or_left->row + ftoprows : 0) && (row > 0 || row_hidden[i]); i--)
                {
                    strow--;
                    if (row_hidden[i])
                        continue;
    ...
    if (FullUpdate || standlast)
    {
        ...
        for (row = RESROW, i = (ftoprows && strow >= fr->or_left->row ? fr->or_left->row : strow); i <= mxrow; i++)
        {
        ...
        for (col = rescol, i = (fleftcols && stcol >= fr->or_left->col ? fr->or_left->col : stcol); i <= mxcol; i++)
        {
            register int k;
    ...
    if (showrange || anychanged || FullUpdate || standlast)
    {
        ...
        for (row = (ftoprows && strow >= fr->or_left->row ? fr->or_left->row : strow), r = RESROW; row <= mxrow; row++)
        {
            int c = rescol;
            int do_stand = 0;
            int fieldlen;
            ...
            for (pp = ATBL(tbl, row, col = (fleftcols && stcol >= fr->or_left->col ? fr->or_left->col : stcol));
                 col <= mxcol; pp += nextcol - col, col = nextcol, c += fieldlen)
            {
                 ...
                 */
                if ((showrange && (!showneed) && (!showexpr) && (row >= minsr) && (row <= maxsr) && (col >= minsc) &&
                     (col <= maxsc)) ||
                    (showneed && (*pp) && ((*pp)->flags & is_valid) &&
                     (((*pp)->flags & is_strexpr) || !((*pp)->expr))) ||
                    (showexpr && (*pp) && ((*pp)->expr)) || (shownote && (*pp) && ((*pp)->nrow >= 0)))
                {
                ...
                if ((*pp) && (((*pp)->flags & is_changed || FullUpdate) || do_stand))
                {
                    ...
                    else if (showexpr && ((*pp)->expr))
                    {
                        ...
                        linelim = -1;
                        showstring(line, /* leftflush = */ 1, /* hasvalue = */ 0, row, col, &nextcol, mxcol, &fieldlen,
                    ...
                    else
                    {
                            ...
                            *field = '\0';
                            ...
                            if (cfmt)
                            {
                                if (*cfmt == ctl('d'))
                                {
                                    ...
                                    (void)format(cfmt, precision[col], (*pp)->v, field, sizeof(field));
                            }
                            else
                            {
                                (void)engformat(realfmt[col], fwidth[col] - note, precision[col], (*pp)->v, field,
                                                sizeof(field));
                            }
                            ...
                            {
                                if (cfmt && *cfmt != ctl('d'))
                                    for (i = 0; i < fwidth[col] - (int)strlen(field) - note; i++)
                                        (void)addch(' ');
                                if (note)
                                {
                                    attr_t attr;
                        ...
                        if ((*pp)->label)
                        {
                            showstring((*pp)->label, (*pp)->flags & (is_leftflush | is_label), (*pp)->flags & is_valid,
                                       row, col, &nextcol, mxcol, &fieldlen, r, c, fr, frightcols, flcols, frcols);
                        }
                        else /* repaint a blank cell: */
                            if ((((do_stand || !FullUpdate) && ((*pp)->flags & is_changed)) ||
    ...
    else
    {
        ...
        { /* show top line */
            ...
            if ((p1 = *ATBL(tbl, currow, curcol)) && p1->nrow > -1)
                printw("{*%s} ", r_name(p1->nrow, p1->ncol, p1->nlastrow, p1->nlastcol));
