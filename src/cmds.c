...
void duprow(void)
{
    ...
    if (currow >= maxrows - 1 || (!fr && maxrow >= maxrows - 1) || (fr && fr->or_right->row >= maxrows - 1))
    {
 ...
 */
void insertrow(int narg, int delta)
{
    int r, c, i;
    ...
    {
        move_area(currow + narg + delta, fr->or_left->col, currow + delta, fr->or_left->col, fr->or_right->row,
                  fr->or_right->col);
        ...
        { /* update all marked cells */
            if (savedrow[i] >= currow + delta && savedcol[i] >= fr->or_left->col && savedcol[i] <= fr->or_right->col)
                savedrow[i] += narg;
            if (savedstrow[i] >= currow + delta && savedstcol[i] >= fr->or_left->col &&
                savedstcol[i] <= fr->or_right->col)
                savedstrow[i] += narg;
        }
        if (gs.g_row >= currow + delta && gs.g_col >= fr->or_left->col && gs.g_col <= fr->or_right->col)
            gs.g_row += narg;
        if (gs.g_lastrow >= currow + delta && gs.g_lastcol >= fr->or_left->col && gs.g_lastcol <= fr->or_right->col)
            gs.g_lastrow += narg;
        if (gs.strow >= currow + delta && gs.stcol >= fr->or_left->col && gs.stcol <= fr->or_right->col)
            gs.strow += narg;
        for (r = 0; r <= maxrow; r++)
        {
            for (c = 0; c <= maxcol; c++)
            {
                ...
                if (*pp)
                {
                    if ((*pp)->nrow >= currow + delta && (*pp)->ncol >= fr->or_left->col &&
                        (*pp)->ncol <= fr->or_right->col)
                        ((*pp)->nrow) += narg;
                    if ((*pp)->nlastrow >= currow + delta && (*pp)->nlastcol >= fr->or_left->col &&
    ...
    fix_ranges(currow + narg * (1 - delta), -1, currow + narg * (1 - delta), -1, delta ? 0 : narg, delta ? narg : 0);
    currow += delta;
    FullUpdate++;
    modflg++;
}
 ...
 * is 0; after if it is 1.
 */
void insertcol(int narg, int delta)
{
    int r, c;
    ...
    if (!delta && (fr = find_frange(currow, curcol)) && fr->ir_left->col == curcol + narg)
    ...
    fix_ranges(-1, curcol + narg * (1 - delta), -1, curcol + narg * (1 - delta), delta ? 0 : narg, delta ? narg : 0);
    curcol += delta;
    FullUpdate++;
    modflg++;
}
...
void deleterow(register int narg)
{
    int i;
    ...
    {
        rs = rs > 0 ? rs : 0;
        (void)sprintf(buf, "Can't delete %d row%s %d row%s left", narg, (narg != 1 ? "s," : ","), rs,
                      (rs != 1 ? "s" : ""));
        error(buf);
        return;
    }
    if (fr)
    {
        if (any_locked_cells(currow, fr->or_left->col, currow + narg - 1, fr->or_right->col))
        ...
        else
        {
            ...
            sync_refs();
            erase_area(currow, fr->or_left->col, currow + narg - 1, fr->or_right->col, 0);
            ...
            for (p = delbuf[dbidx]; p; p = p->next)
                p->flags &= ~may_sync;
            if (currow + narg > fr->ir_right->row && fr->ir_right->row >= currow)
                fr->ir_right = lookat(currow - 1, fr->ir_right->col);
            if (currow + narg > fr->or_right->row)
                fr->or_right = lookat(currow - 1, fr->or_right->col);
            else
                move_area(currow, fr->or_left->col, currow + narg, fr->or_left->col, fr->or_right->row,
                          fr->or_right->col);
            ...
            for (i = 0; i < 37; i++)
            {
                if (savedcol[i] >= fr->or_left->col && savedcol[i] <= fr->or_right->col)
                {
                ...
                if (savedstcol[i] >= fr->or_left->col && savedstcol[i] <= fr->or_right->col)
                {
...
void yankrow(int narg)
{
    ...
    {
        rs = rs > 0 ? rs : 0;
        (void)sprintf(buf, "Can't yank %d row%s %d row%s left", narg, (narg != 1 ? "s," : ","), rs,
                      (rs != 1 ? "s" : ""));
        error(buf);
        return;
    }
    ...
    if (fr)
    {
        yank_area(currow, fr->or_left->col, currow + narg - 1, fr->or_right->col);
    }
    else
    {
...
void yankcol(int narg)
{
    ...
    {
        cs = cs > 0 ? cs : 0;
        (void)sprintf(buf, "Can't yank %d column%s %d column%s left", narg, (narg != 1 ? "s," : ","), cs,
                      (cs != 1 ? "s" : ""));
        error(buf);
        return;
    }
...
void pullcells(int to_insert)
{
    ...
    { /* Do an exchange. */
         ...
         */
        erase_area(minrow + deltar, mincol + deltac, mxrow + deltar, mxcol + deltac, 0);
        tmpbuf = delbuf[dbidx];
    ...
    {
        erase_area(minrow + deltar, mincol + deltac, minrow + deltar + mxcol - mincol, mincol + deltac + mxrow - minrow,
                   0);
        sync_refs();
        flush_saved();
    }
    ...
    for (p = delbuf[dbidx]; p; p = p->next)
    {
        if (to_insert == 't') /* Transpose rows and columns while pulling. */
            n = lookat(minrow + deltar + p->col - mincol, mincol + deltac + p->row - minrow);
        else
...
void closecol(int narg)
{
    ...
    {
        cs = cs > 0 ? cs : 0;
        (void)sprintf(buf, "Can't delete %d column%s %d column%s left", narg, (narg != 1 ? "s," : ","), cs,
                      (cs != 1 ? "s" : ""));
        error(buf);
        return;
    }
...
void doend(int rowinc, int colinc)
{
    ...
    if (VALID_CELL(p, currow, curcol))
    {
        r = currow + rowinc;
        c = curcol + colinc;
        if (r >= 0 && r < maxrows && c >= 0 && c < maxcols && !VALID_CELL(p, r, c))
        {
            currow = r;
            curcol = c;
        }
...
void doformat(int c1, int c2, int w, int p, int r)
{
    ...
    if (c1 >= maxcols && !growtbl(GROWCOL, 0, c1))
        c1 = maxcols - 1;
    if (c2 >= maxcols && !growtbl(GROWCOL, 0, c2))
        c2 = maxcols - 1;
...
void formatcol(int narg)
{
    int c, i;
    int mf = modflg;
    int *oldformat;

    error("Current format is %d %d %d", fwidth[curcol], precision[curcol], realfmt[curcol]);
    refresh();
    ...
    c = nmgetch();
    while (c >= 0 && c != ctl('m') && c != 'q' && c != ESC && c != ctl('g') && linelim < 0)
    {
        if (c >= '0' && c <= '9')
            ...
            case ' ':
                ...
                else
                {
                    ...
                    (void)sprintf(line + strlen(line), "%s ", coltoa(curcol + narg - 1));
                }
                linelim = (int)strlen(line);
                insert_mode();
                error("Current format is %d %d %d", fwidth[curcol], precision[curcol], realfmt[curcol]);
...
void print_options(FILE *f)
{
    if (autocalc && !autoinsert && !autowrap && !cslop && !optimize && !rndtoeven && propagation == 10 &&
        calc_order == BYROWS && !numeric && prescale == 1.0 && !extfunc && showtop && tbl_style == 0 && craction == 0 &&
        pagesize == 0 && rowlimit == -1 && collimit == -1 && !color && !colorneg && !colorerr)
    ...
    if (tbl_style)
        (void)fprintf(f, " tblstyle = %s",
                      tbl_style == TBL      ? "tbl"
                      : tbl_style == LATEX  ? "latex"
                      : tbl_style == SLATEX ? "slatex"
                      : tbl_style == TEX    ? "tex"
                      : tbl_style == FRAME  ? "frame"
                                            : "0");
    if (craction)
...
void printfile(char *fname, int r0, int c0, int rn, int cn)
{
    ...
    if (fname)
    {
        ...
        if ((strcmp(fname, curfile) == 0) && !yn_ask("Confirm that you want to destroy the data base: (y,n)"))
        {
            return;
        }
    ...
    if (!pline && (pline = scxmalloc((unsigned)(FBUFLEN * ++fbufs_allocated))) == (char *)NULL)
    {
    ...
    for (row = r0; row <= rn; row++)
    {
        ...
        pline[plinelim = 0] = '\0';
        for (pp = ATBL(tbl, row, col = c0); col <= cn; pp += nextcol - col, col = nextcol, c += fieldlen)
        {
            ...
            if (*pp)
            {
                ...
                {
                    if ((pline = scxrealloc((char *)pline, (unsigned)(FBUFLEN * ++fbufs_allocated))) == NULL)
                    {
                ...
                {
                    while ((unsigned)(plinelim + fwidth[col]) > (fbufs_allocated * FBUFLEN))
                    {
                        if ((pline = ((char *)scxrealloc((char *)pline, (unsigned)(FBUFLEN * ++fbufs_allocated)))) ==
                            NULL)
                        {
                            error("Realloc failed in printfile()");
                            return;
                    ...
                    else
                    {
                        ...
                        cfmt = (*pp)->format ? (*pp)->format
                               : (realfmt[col] >= 0 && realfmt[col] < COLFORMATS && colformat[realfmt[col]])
                        ...
                        if (cfmt)
                        {
                            ...
                            if (*cfmt == ctl('d'))
                            {
                                ...
                                sprintf(pline + plinelim, "%-*s", fwidth[col], field);
                            }
                            else
                            {
                                format(cfmt, precision[col], (*pp)->v, field, sizeof(field));
                                (void)sprintf(pline + plinelim, "%*s", fwidth[col], field);
                            }
                        }
                        else
                        {
                ...
                if ((s = (*pp)->label))
                {
                    ...
                    if (*s == '\\' && *(s + 1) != '\0')
                        slen = fwidth[col];
                    while (slen > fieldlen && nextcol <= cn && !((nc = lookat(row, nextcol))->flags & is_valid) &&
                           !(nc->label))
                    {
                    ...
                    start = (*pp)->flags & is_leftflush ? pline + c : pline + c + fieldlen - slen;
...
{
    if (*str == '\\')
        str++; /* delete wheeling string operator, OK? */
    while (*str)
    {
        if (((tbl_style == LATEX) || (tbl_style == SLATEX) || (tbl_style == TEX)) &&
            ((*str == delim) || (*str == '$') || (*str == '#') || (*str == '%') || (*str == '{') || (*str == '}') ||
             (*str == '&')))
            putc('\\', f);
        putc(*str, f);
        str++;
...
struct enode *copye(register struct enode *e, int Rdelta, int Cdelta, int r1, int c1, int r2, int c2, int transpose)
{
    ...
    else if (e->op & REDUCE)
    {
        ...
        ret->op = e->op;
        newrow = e->e.r.left.vf & FIX_ROW || e->e.r.left.vp->row < r1 || e->e.r.left.vp->row > r2 ||
                         e->e.r.left.vp->col < c1 || e->e.r.left.vp->col > c2
                     ? e->e.r.left.vp->row
                 : transpose ? r1 + Rdelta + e->e.r.left.vp->col - c1
                             : e->e.r.left.vp->row + Rdelta;
        newcol = e->e.r.left.vf & FIX_COL || e->e.r.left.vp->row < r1 || e->e.r.left.vp->row > r2 ||
                         e->e.r.left.vp->col < c1 || e->e.r.left.vp->col > c2
                     ? e->e.r.left.vp->col
        ...
        newrow = e->e.r.right.vf & FIX_ROW || e->e.r.right.vp->row < r1 || e->e.r.right.vp->row > r2 ||
                         e->e.r.right.vp->col < c1 || e->e.r.right.vp->col > c2
                     ? e->e.r.right.vp->row
                 : transpose ? r1 + Rdelta + e->e.r.right.vp->col - c1
                             : e->e.r.right.vp->row + Rdelta;
        newcol = e->e.r.right.vf & FIX_COL || e->e.r.right.vp->row < r1 || e->e.r.right.vp->row > r2 ||
                         e->e.r.right.vp->col < c1 || e->e.r.right.vp->col > c2
                     ? e->e.r.right.vp->col
    ...
    else
    {
        ...
        case 'v': {
            int newrow, newcol;
            if (range && e->e.v.vp->row >= range->e.r.left.vp->row && e->e.v.vp->row <= range->e.r.right.vp->row &&
                e->e.v.vp->col >= range->e.r.left.vp->col && e->e.v.vp->col <= range->e.r.right.vp->col)
            {
            ...
            {
                newrow = e->e.v.vf & FIX_ROW || e->e.v.vp->row < r1 || e->e.v.vp->row > r2 || e->e.v.vp->col < c1 ||
                                 e->e.v.vp->col > c2
                             ? e->e.v.vp->row
                         : transpose ? r1 + Rdelta + e->e.v.vp->col - c1
                                     : e->e.v.vp->row + Rdelta;
                newcol = e->e.v.vf & FIX_COL || e->e.v.vp->row < r1 || e->e.v.vp->row > r2 || e->e.v.vp->col < c1 ||
                                 e->e.v.vp->col > c2
                             ? e->e.v.vp->col
...
#ifdef DOBACKUPS
        if (rfd == NULL && !backup_file(efname) && (yn_ask("Could not create backup copy.  Save anyway?: (y,n)") != 1))
            return (0);
#endif
 ...
 */
void copyent(register struct ent *n, register struct ent *p, int dr, int dc, int r1, int c1, int r2, int c2,
             int special)
{
    if (!n || !p)
    {
...
int writefile(char *fname, int r0, int c0, int rn, int cn)
{
    ...
    if (scext != NULL)
    {
        ...
        else if (strlen(tfname) > strlen(scext) + 1 && tfname[strlen(tfname) - strlen(scext) - 1] == '.' &&
...
void forwcol(int narg)
{
    while (--narg >= 0)
    {
        if (curcol < maxcols - 1)
            curcol++;
        else if (!growtbl(GROWCOL, 0, narg)) /* get as much as needed */
            break;
        else
            curcol++;
...
void forwrow(int narg)
{
    while (--narg >= 0)
    {
        if (currow < maxrows - 1)
            currow++;
        else if (!growtbl(GROWROW, narg, 0)) /* get as much as needed */
            break;
        else
            currow++;
...
void dotick(int tick)
{
    ...
    if (c == '`' || c == '\'')
        c = 0;
    else if (!(((c -= ('a' - 1)) > 0 && c < 27) || ((c += ('a' - '0' + 26)) > 26 && c < 37)))
    {
                ...
                int flcols, int frcols) /* width of left and right sides of frame */
{
    ...
    if (c + fieldlen == rescol + flcols && nextcol < stcol)
        nextcol = stcol;
    if (frightcols && c + fieldlen + fwidth[nextcol] >= COLS - 1 - frcols &&
        nextcol < fr->or_right->col - frightcols + 1)
        nextcol = fr->or_right->col - frightcols + 1;
    while ((slen > fieldlen) && (nextcol <= mxcol) && !((nc = lookat(row, nextcol))->flags & is_valid) &&
           !(nc->label) && (cslop || find_crange(row, nextcol) == cr))
    {
